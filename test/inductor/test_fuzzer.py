...
class TestConfigFuzzer(TestCase):
    ...
    def test_fuzzer_inductor_calling_compile(self, compile):
        """
        This test makes sure that the fuzzer is actually testing with inductor.
        """
        def create_key_1():
            def myfn():
                return True
        ...
        num_attempts = 3
        results = fuzzer.bisect(num_attempts=num_attempts, p=0.5)
        self.assertEqual(compile.call_count, num_attempts)

    @unittest.skipIf(sys.version_info < (3, 10), "python < 3.10 not supported")
    @patch('torch._inductor.compile')
    def test_fuzzer_inductor_calling_compile_fx_inner(self, compile):
        """
        This test makes sure that the fuzzer is actually testing with inductor.
        """
        def create_key_1():
